...
interface CurrencyProps {
    ...
    setActiveCurrency: React.Dispatch<
        React.SetStateAction<Currency>
    >;
    treasury: Stronghold["treasury"];
    updatingTreasury: boolean;
    updateTreasury: (currency: Currency, value: number, method: "increase" | "decrease") => Promise<void>;
}
...
export default function CurrencyElement({
    ...
    async function adjustCurrency(method: "increase" | "decrease", currency: Currency) {
    ...
    >
        ...
        >
            <span className={`${styles.currencyAmount} ${animatedCurrency == currency ? styles.animated : ""}`}>
                {treasury[currency]}
            </span>
